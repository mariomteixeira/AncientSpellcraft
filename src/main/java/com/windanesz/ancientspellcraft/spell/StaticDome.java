...
/** Cupula eletrificada em volta do caster (1.12.2 StaticDome; colocada de uma vez). */
public class StaticDome extends Spell {
    ...
    public boolean cast(PlayerCastContext ctx) {
        ...
        if (!ctx.world().isClientSide) {
            ...
            for (BlockPos pos : shellBlocks(ctx, property(DefaultProperties.BLAST_RADIUS).intValue())) {
                if (pos.getY() < ctx.caster().getY()) continue; // cupula, nao esfera
                TemporaryBlockEntity.place(ctx.caster(), ctx.world(), ASBlocks.LIGHTNING_BLOCK.get(), pos, lifetime);
